...
WORKSPACE_CAM_PATH = "/dev/video2"     # Logitech C270 (open by PATH, not index)
...
class WebcamEEJointTeleop(Teleoperator):
    ...
    def __init__(self, config, controller: WebcamEEController, source: WebcamSource, robot=None):
    ...
    def get_action(self) -> dict:
        ...
        if frame is not None:
            ...
            cv2.putText(frame, f"hand: {state}", (8, 52),
...
def main():
    ...
    controller = WebcamEEController(robot, kin, cfg, use_oak=True)
    ...
    if resume:
        ...
        dataset = LeRobotDataset.resume(HF_REPO_ID, root=DATASET_ROOT, image_writer_threads=4)
        print(f"[record] RESUMING existing dataset: {dataset.num_episodes} episode(s) already there")
    else:
    ...
    # Silence the repetitive "Record loop running slower than target FPS" warning: it's expected on
    # this serial bus under gripping load (see FPS note) and the frames still record. Filtered, not
    # raised to ERROR, so real warnings still show.
    import logging
    ...
    def wait_for_start(ep_human):
        """Keep the arm live (follow your hand) and wait for START. Returns False if the user quits."""
        teleop.set_status(f"READY  ep {ep_human}/{NUM_EPISODES}   ->  RIGHT/SPACE = start    ESC = quit")
        teleop.last_key = 255
        events["exit_early"] = False
        while True:
            robot.send_action(teleop.get_action())
            if getattr(source, "oak_failed", False):   # OAK crashed -> end cleanly (session discarded)
    ...
    try:
        ...
        teleop.disconnect()
        robot.disconnect()
        if listener is not None:        # None in headless mode (pynput missing)
            listener.stop()
        dataset.finalize()

        # --- include or discard this session ---
        if ep == 0:
            keep = False
            print("\n[record] no episodes recorded this session.")
        else:
            try:
                ans = input(f"\n>>> Include this session's {ep} episode(s) in the dataset? [Y/n]: ").strip().lower()
            except EOFError:
                ans = "y"
            keep = ans not in ("n", "no")

        if keep:
            print(f"[record] KEPT -- dataset now has {dataset.num_episodes} episode(s) at {DATASET_ROOT}")
            if os.path.exists(backup_dir):
                shutil.rmtree(backup_dir)            # session committed; drop the snapshot
        else:
            shutil.rmtree(DATASET_ROOT, ignore_errors=True)
            if resume and os.path.exists(backup_dir):
                shutil.move(backup_dir, DATASET_ROOT)   # restore the pre-session dataset exactly
                print(f"[record] DISCARDED session -- restored previous {already} episode(s).")
            else:
