...
def dwell(robot, *, body: dict[str, float], gripper_pos: float, seconds: float, hz: float,
          arm_enabled: bool, started_at: float, max_current: float, samples: list) -> list:
    """Hold one aperture, sampling telemetry, and stop early on a current spike."""
    period = 1.0 / hz
    collected = []
...
def sweep(robot, *, body: dict[str, float], step_size: float, direction: str,
    ...
    previous = dwell(robot, body=body, gripper_pos=commanded, seconds=args.dwell_s, hz=args.hz,
                     ...
                     max_current=args.max_current, samples=samples)
...
def run(args) -> tuple[dict, list]:
    ...
    try:
        ...
        if not args.yes:
            input("Close the jaw on the carton, then press Enter to begin (Ctrl-C aborts): ")
...
def main():
    ...
    ap.add_argument("--max-current", type=float, default=400.0,
